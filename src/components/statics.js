  ...
  retrieveStatic() {
    StaticDataService.getAll()
      .then((response) => {
        ...
        for (let i = 0; i < length; i++) {
          if (
            response.data[i].APP_VeloRel === "" &&
            objVeloCount.hasOwnProperty(response.data[i].TaggedPitchType)
          ) {
          ...
          objVelo[response.data[i].TaggedPitchType] += Number(
            response.data[i].APP_VeloRel
          );
        }
        ...
        for (let i = 0; i < Object.keys(objVelo).length; i++) {
          tmpVelo.push(
            Math.round(
              (Object.values(objVelo)[i] / Object.values(objVeloCount)[i] +
                Number.EPSILON) *
                10
            ) / 10
          );
        ...
        for (let i = 0; i < length; i++) {
          ...
          if (
            response.data[i].PitchCode === "Strk-C" ||
            response.data[i].PitchCode === "Strk-S" ||
            response.data[i].PitchCode === "Foul" ||
            response.data[i].PitchCode === "In-Play"
          )
        ...
        for (let i = 0; i < Object.keys(objStrike).length; i++) {
          tmpStrike.push(
            Math.round(
              (Object.values(objStrike)[i] / Object.values(objTypeCount)[i] +
                Number.EPSILON) *
                100
            ) / 1
          );
        ...
        for (let i = 0; i < length; i++) {
          ...
          if (
            response.data[i].PlayResult !== "" ||
            response.data[i].KorBB !== ""
          )
        ...
        for (let i = 0; i < length; i++) {
          ...
          if (
            response.data[i].KorBB === "HBP" ||
            response.data[i].KorBB === "BB" ||
            response.data[i].KorBB === "IBB"
          )
        ...
        for (let i = 0; i < length; i++) {
          ...
          if (
            response.data[i].PlayResult === "1B" ||
            response.data[i].PlayResult === "2B" ||
            response.data[i].PlayResult === "3B" ||
            response.data[i].PlayResult === "HR"
          )
        ...
        for (let i = 0; i < length; i++) {
          ...
          if (
            response.data[i].KorBB === "K" ||
            response.data[i].KorBB === "Ks" ||
            response.data[i].KorBB === "K-DO"
          )
        ...
        for (let i = 0; i < Object.keys(objPA).length; i++) {
          tmpBABIP.push(
            Math.round(
              ((Object.values(objH)[i] - Object.values(objHR)[i]) /
                (Object.values(objPA)[i] -
                  (Object.values(objBB)[i] +
                    Object.values(objSF)[i] +
                    Object.values(objSH)[i]) -
                  Object.values(objSO)[i] -
                  Object.values(objHR)[i] +
                  Object.values(objSF)[i]) +
                Number.EPSILON) *
                1000
            ) / 1000
        ...
        let objArr = [];
        for (let i = 0; i < length; i++) {
          objArr.push(response.data[i].APP_KZoneY);
        }
          ...
          <Table striped bordered hover>
            ...
            <tbody>
              <tr>
                <td>
                  {pitchType &&
                    Array.from(new Set(pitchType)).map((v) => (
                      <td className="list-group-item">{v}</td>
                    ))}
                </td>
                <td>
                  {count &&
                    count.map((v) => <td className="list-group-item ">{v}</td>)}
                </td>
                <td>
                  {velo &&
                    velo.map((v) => <td className="list-group-item ">{v}</td>)}
                </td>
                <td>
                  {strike &&
                    strike.map((v) => (
                ...
                <td>
                  {BABIP &&
                    BABIP.map((v) => <td className="list-group-item ">{v}</td>)}
                </td>
              </tr>
            </tbody>
          </Table>
